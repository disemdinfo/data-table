...
function prepareColumn(_ref) {
  ...
  return _extends({}, column, {
    ...
    headerStyle: _extends({}, params.headerStyle, column.headerStyle),
    style: _extends({}, params.style, column.style)
  });
}

function prepareColumns(_ref2) {
  var columns = _ref2.columns,
      params = _objectWithoutProperties(_ref2, ['columns']);
  ...
  Object.keys(columns).filter(function (key) {
    ...
    cols[key] = prepareColumn(_extends({ columns: columns, column: column, key: key }, params));
  });
...
function prepareGroupColumns(_ref3) {
  var columns = _ref3.columns,
      params = _objectWithoutProperties(_ref3, ['columns']);
  ...
  Object.keys(columns).filter(function (key) {
    ...
    if (column.columns) {
      column.columns = prepareColumns(_extends({ columns: column.columns }, params));
    } else {
      column = { columns: _defineProperty({}, key, prepareColumn(_extends({ columns: columns, column: column, key: key }, params))) };
    }
  ...
  _createClass(DataTable, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      if (rows !== this.props.rows) {
        this.setState(this.getState(nextProps), function () {
    ...
    value: function getState(params) {
      ...
      var cols = (0, _clone2.default)(hasGroup ? prepareGroupColumns({ columns: columns, params: params }) : prepareColumns({ columns: columns, params: params }));
    ...
    key: 'getFilteredRows',
    value: function getFilteredRows(_ref4) {
      var value = _ref4.value,
          column = _ref4.column,
          columns = _ref4.columns;
      var startRows = this.startRows;

      var cols = (0, _utils.getColumns)(columns);
      if (column) column.searchValue = value;
      var filteredColumns = Object.keys(cols).filter(function (key) {
        return cols[key].searchValue;
      }).map(function (key) {
        return cols[key];
      });
    ...
    value: function onSearch(_ref5) {
      ...
      var data = this.getFilteredRows({ value: value, column: column, columns: columns });
